...
\begin{itemize}[nosep]
    \item A patient is predicted \emph{positive} if the model outputs at least one detection with confidence $>0.3$.
...
Figure~\ref{fig:val_ap} shows the validation AP@0.5 over training epochs. FCOS achieves the highest AP@0.5 (1.2\% at epoch 2) and maintains non-zero detection across most epochs. Faster R-CNN shows a brief peak in epoch 1 (AP@0.5 = 0.74\%) before declining to zero, while RetinaNet produces no meaningful detections throughout training.
...
\midrule
FCOS (Paper) & 77.0 &  0.0 &   0.0 &  0.0 \\
RetinaNet    & 77.0 &  0.0 &   0.0 &  0.0 \\
Faster R-CNN & 77.0 &  0.0 &   0.0 &  0.0 \\
\bottomrule
\end{tabular}
\vspace{0.3em}

\footnotesize\textit{Note: None of the models produce detections above the 0.3 confidence threshold, defaulting to all-negative predictions. The 77\% accuracy reflects the class distribution (77\% of patients are negative).}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Anchor-free $=$ faster convergence.} FCOS achieves non-zero AP by epoch 2, while RetinaNet never produces meaningful detections and Faster R-CNN only briefly achieves AP@0.5 = 0.74\% in its first epoch before declining to zero.

    \item \textbf{Patient-level classification reveals insufficient training.} At a confidence threshold of 0.3, none of the three models produce detections above this threshold, resulting in all-negative predictions for every patient. The 77\% accuracy for all models simply reflects the class distribution (77\% of patients are negative). This indicates that while FCOS and Faster R-CNN learn to detect some pneumonia regions (non-zero AP), their detection confidence remains below 0.3 after only 5 epochs of training on 500 patients.
\end{enumerate}
